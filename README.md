...
- **v0.5.15** (2026-07-08)：參考 `obsidian-md-notes-viewer` 檢視器架構，於網站「系統設定」中整合「全生命週期開發架構互動流程圖檢視器 (`LifecycleFlowchartModal`)」，支援滑鼠滾輪放大/縮小、適應視窗與左鍵拖曳全景瀏覽。
- **v0.5.14** (2026-07-08)：新增「專案開發全生命週期流程與關鍵字導航 (Lifecycle Roadmap)」UI 卡片，明確標示 5 大開發階段與指令對應關係，並釐清 `#狀態` 與 `#更新/#收工` 之執行職責區隔。
- **v0.5.13** (2026-07-08)：新增「`#更新`」核心工作流指令（執行效果與 `#收工` 完全一致，適用於階段性隨時存檔同步），並完成網站全域技能及資料庫狀態同步。
